...
public:
  ...
  /// If you have shown the statistics
...
// Implementation
template <class T>
grid_statistics<T>::grid_statistics(
  const std::vector<std::pair<const std::string, T>>& v,
...
template <class T> void grid_statistics<T>::clear()
{
  statistics<T>::clear();
  m_keys.clear();
...
template <class T>
const T grid_statistics<T>::set(const std::string& key, T value)
{
...
template <class T>
grid* grid_statistics<T>::show(bool hide_row_labels, bool hide_col_labels)
{
  if (GetNumberRows() > 0)
  {
    DeleteRows(0, GetNumberRows());
...
template <class T> void grid_statistics<T>::build_popup_menu(menu& menu)
{
  menu.style().reset();
