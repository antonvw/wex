  ...
  const lexer&            lexer)
{
  ...
  while (!in.empty())
  {
    ...
    {
      out +=
        lexer.make_single_line_comment(line, fill_out_with_space, fill_out) +
        "\n";
  ...
  dir::type_t        type)
{
  ...
  {
    file_dialog dlg(window_data().style(style).title(caption), wildcards);
  ...
  open_files(
    ...
    hexmode ? stc_data(data).flags(
                stc_data::window_t().set(stc_data::WIN_HEX),
                control_data::OR) :
              data,
    type);
}
...
bool wex::sort_selection(stc* stc, string_sort_t sort_t, size_t pos, size_t len)
{
  bool error = false;
