...
{ 
  namespace ast
  {
    ...
    struct eval
    {
      eval(const wex::ex* ex) : m_ex(ex) {;};

      auto operator() (nil) const {BOOST_ASSERT(0); return 0;}
      ...
      int operator() (int lhs, operation const& x) const
      {
          ...
          x.operator_)
        {
          ...
          case '<': return lhs << rhs;
          case '>': return lhs >> rhs;
          case '/':  if (rhs == 0) throw std::overflow_error("divide by zero");
            return lhs / rhs;
        }
        BOOST_ASSERT(0);
        return 0;
  ...
  public:
    std::tuple<int, std::string> eval(
      const wex::ex* ex, const std::string& text)
    {
      try
      {
