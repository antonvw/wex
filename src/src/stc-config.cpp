...
void wex::stc::config_get()
{
  ...
  SetMarginWidth(m_margin_divider_number, iv.find<int>(_("stc.margin.Divider")));
...
{
  m_config_items = new std::vector < item > ({{"stc-notebook", {
    {_("General"),
      {{"stc-subnotebook", {
        {_("Page1"), 
          {{{_("stc.End of line"),
          ...
          {_("stc.Search engine"), item::COMBOBOX, 
    ...
    {_("Folding"),
      {{_("stc.Indentation guide"), item::CHECKBOX},
       ...
       {_("stc.Fold flags"), {
           ...
           {wxSTC_FOLDFLAG_LINEBEFORE_CONTRACTED, def(_("Line before contracted"))},
           {wxSTC_FOLDFLAG_LINEAFTER_EXPANDED, _("Line after expanded")},
           {wxSTC_FOLDFLAG_LINEAFTER_CONTRACTED, def(_("Line after contracted"))},
           {wxSTC_FOLDFLAG_LEVELNUMBERS, _("Level numbers")}},
           false}}},
    {_("Directory"),
      {{_("stc.Include directory"), listview_data().type(listview_data::FOLDER).
