...
void wex::statusbar::handle(wxMouseEvent& event, const statusbar_pane& pane)
{
  if (event.LeftUp())
  {
    if (pane.get_name() == "PaneText")
    {
      ...
      if (item_dialog(
        v_i,
        window_data().title("Statusbar Panes"), 0, 2).ShowModal() == wxID_OK)
      {
        ...
        for (const auto& it : m_panes)
        {
          v_p.push_back({
            ...
            it.help_text(),
            pane_styles().style(
              config("statusbar.styles." + it.get_name()).get(
                pane_styles().find(it.GetStyle()))),
            it.is_shown()});
        }
...
// The second pane is shown_pane_no, or FIELD_NOT_SHOWN if the pane is not shown,
// to be used as index in wxwidgets panes.
// The third pane the pane_no as index in the panes vector.
...
bool wex::statusbar::set_statustext(
  const std::string& text, const std::string& pane)
{
...
bool wex::statusbar::show_pane(const std::string& pane, bool show)
{
