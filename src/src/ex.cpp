                ...
                [&](const std::string& command) {
                  ...
                  wex::cmdline cmdline(
                      ...
                      [&](bool on) {
                        if (!modeline)
                          config(_("stc.Edge line"))
                            .set(
                              on ? (long)wxSTC_EDGE_LINE :
                                   (long)wxSTC_EDGE_NONE);
                        else
                    ...
                    {{{"dir", "dir", wex::path::current()},
                     ...
                     {{"ec",
                       "edgecolumn",
                       std::to_string(get_stc()->GetEdgeColumn())},
                      {cmdline::INT,
                       [&](const std::any& val) {
                         if (!modeline)
                           config(_("stc.Edge column"))
                             .set(std::any_cast<int>(val));
                         else
