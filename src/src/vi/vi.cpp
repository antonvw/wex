  ...
  , m_motion_commands{{"h",
                      ...
                      {"G",
                       [&](const std::string& command) {
                         ...
                         {
                           (void)stc_data(get_stc())
                             .control(control_data().line(m_count))
                             .inject();
                         }
                         return 1;
                       }},
...
bool wex::vi::command(const std::string& command)
{
  ...
  if (
    mode().visual() && command.find("'<,'>") == std::string::npos &&
    ex::command(command + "'<,'>"))
  {
    return auto_write();
  }
...
char wex::vi::convert_key_event(const wxKeyEvent& event) const
{
  if (event.GetKeyCode() == WXK_BACK)
    return WXK_BACK;
  if (event.GetKeyCode() == WXK_RETURN && !mode().insert())
    return 'j';
