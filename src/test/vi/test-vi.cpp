...
TEST_CASE("wex::vi")
{
  ...
  SUBCASE("insert")
  {
    ...
    for (auto& it1 : vi->mode().insert_commands())
    {
      ...
      {
        CAPTURE( it1.first);
        change_mode( vi, std::string(1, it1.first), wex::vi_mode::state_t::INSERT);
        change_mode( vi, ESC, wex::vi_mode::state_t::NORMAL);
  ...
  SUBCASE("put")
  {
    ...
    REQUIRE( stc->GetLength() > 100 * sizeof("the chances of anything coming from mars"));
  }
  ...
  SUBCASE("others")
  {
      ...
      "101", "0xf7", "077", "-99"})
    {
      ...
      REQUIRE(!vi->on_char(event));
      CAPTURE( number);
      REQUIRE( stc->GetText().find(number) == std::string::npos);
    }
    
    // Test navigate command keys.
    for (const auto& nav_key : std::vector<int> {
      WXK_BACK, WXK_RETURN, WXK_LEFT, WXK_DOWN, WXK_UP, WXK_RIGHT,
