...
// Added lmRFW for Robot Framework by A. van Wezenbeek 2016
...
Sci_Position Init(Accessor &styler, Sci_PositionU startPos)
{
  // Always backtracks to the start of a line that is not a continuation
  // of the previous line (i.e. start of a bash or rfw command segment)
...
static void ColouriseBashDoc(Sci_PositionU startPos, Sci_Position length, int initStyle,
	...
	int testExprType = 0;
  Sci_Position ln = Init(styler, startPos);
...
// extra state constants for parts of a rfw command segment
#define RFW_CMD_START        BASH_CMD_DELIM + 1
// Spaced KeyWord processing
#define RFW_CMD_SKW_PARTIAL  RFW_CMD_START + 0
// a robot framework keyword
#define RFW_CMD_TESTCASE     RFW_CMD_START + 1
...
static bool IsPipeLine(Sci_Position line, Accessor &styler)
{
  Sci_Position pos = styler.LineStart(line);
  Sci_Position eol_pos = styler.LineStart(line + 1) - 1;
...
static bool IsTabLine(Sci_Position line, Accessor &styler)
{
  Sci_Position pos = styler.LineStart(line);
  Sci_Position eol_pos = styler.LineStart(line + 1) - 1;
...
static void FoldRFWDoc(Sci_PositionU startPos, Sci_Position length, int, WordList *[],
  Accessor &styler)
{
  bool foldComment = styler.GetPropertyInt("fold.comment") != 0;
  bool foldPipes = styler.GetPropertyInt("fold.pipes") != 0;
  bool foldTabs = styler.GetPropertyInt("fold.tabs") != 0;
  bool foldCompact = styler.GetPropertyInt("fold.compact", 1) != 0;
  Sci_PositionU endPos = startPos + length;
  int visibleChars = 0;
  Sci_Position lineCurrent = styler.GetLine(startPos);
  ...
  for (Sci_PositionU i = startPos; i < endPos; i++) {
    ...
    // Pipe folding
    if (foldPipes && atEOL && IsPipeLine(lineCurrent, styler))
    {
      if (!IsPipeLine(lineCurrent - 1, styler)
        && IsPipeLine(lineCurrent + 1, styler))
        levelCurrent++;
      else if (IsPipeLine(lineCurrent - 1, styler)
           && !IsPipeLine(lineCurrent + 1, styler))
        levelCurrent--;
    }

    // Tab folding
    if (foldTabs && atEOL && IsTabLine(lineCurrent, styler))
    {
      if (!IsTabLine(lineCurrent - 1, styler)
        && IsTabLine(lineCurrent + 1, styler))
        levelCurrent++;
      else if (IsTabLine(lineCurrent - 1, styler)
           && !IsTabLine(lineCurrent + 1, styler))
        levelCurrent--;
    }
